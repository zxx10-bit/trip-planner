...
def search_attraction_photo(name: str, city: Optional[str] = None) -> Optional[str]:
    """
    通过高德Web服务API搜索景点真实图片(直接HTTP调用,不依赖MCP)

    Args:
        name: 景点名称
        city: 城市名称(可选,用于避免重名景点)
    ...
    try:
        ...
        params: Dict[str, Any] = {
            ...
            "keywords": name,
            "extensions": "all",  # 需要all才会返回photos字段
            "citylimit": "true",
            "offset": 1,
            "page": 1
        }
        ...
        if data.get("status") != "1":
            print(f"⚠️ 高德图片搜索失败: {data.get('info')}")
            return None
